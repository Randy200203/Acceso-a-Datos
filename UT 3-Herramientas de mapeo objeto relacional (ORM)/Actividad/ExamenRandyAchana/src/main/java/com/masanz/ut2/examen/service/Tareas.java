...
public class Tareas {
    ...
    public Tareas(EntityManagerFactory emf){
        this.emf = emf;
//        ConectionManager.conectar("ut2", "root", "root");
    ...
    public EmpresasDTO buscarEmpresa(String nombreEmpresa, String sector) {
        ...
        EmpresasDTO result = empresaDao.buscarEmpresa(nombreEmpresa, sector);
    ...
    public List<EmpresasDTO> obtenerEmpresas() {
        ...
        List<EmpresasDTO> result = empresaDao.obtenerEmpresas();
        return result;
    }
    ...
    public EmpresasDTO obtenerEmpresa(int id) {
        EmpresasDTO result = empresaDao.obtenerEmpresa(id);
        return result;
    }
    ...
    public List<ProductosDTO> obtenerProductos() {
        List<ProductosDTO> result = productoDao.obtenerProductos();
        return result;
    }
    ...
    public ProductosDTO obtenerProducto(int id) {
        ProductosDTO result = productoDao.obtenerProducto(id);
        return result;
    }
    ...
    public List<StockDTO> obtenerStockEmpresa(EmpresasDTO empresaLogeada) {
        List<StockDTO> result = stockDao.obtenerStockEmpresa(empresaLogeada.getId());
        return result;
    }
    ...
    public void comprarProductos(EmpresasDTO empresaLogeada, HashMap<Integer, Integer> productosParaComprar) {
        int idEmpresa = -1;
        ...
        if(productosParaComprar!=null && productosParaComprar.size()>0){
            ...
            while(iterator.hasNext()){
                ...
                ProductosDTO producto = productoDao.obtenerProducto(idProducto);
                int costeProductoUnidad = producto.getValor();

                StockDTO stock = stockDao.buscarStock(idEmpresa, idProducto);
                if (stock != null) {
                    stockDao.actualarStock(stock.getId(), stock.getCantidad() + cantidad);
                } else {
                    int id = stockDao.obtenerStockCompleto().size();
                    id++;
                    stockDao.insertarStock( idEmpresa, idProducto, cantidad);
                }
    ...
    public void venderProductos(EmpresasDTO empresaLogeada, HashMap<Integer, Integer> productosParaVender) {
